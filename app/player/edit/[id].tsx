...
export default function EditPlayerScreen() {
  ...
  const { clubs, terrains, matches, loading: appLoading } = useAppData();
  const { getPlayerById, updatePlayer, deletePlayer } = useAppActions();
  ...
  const progressLabel = !name.trim() ? t('player', 'nameIsRequired') : !location.city ? (language === 'fr' ? 'Localisation requise' : 'Location required') : t('tournament', 'readyToCreate');
  ...
  useEffect(() => {
    if (player) {
      ...
      setLocation({ address: '', city: player.location?.city || '', country: player.country || 'France', latitude: player.location?.latitude || 0, longitude: player.location?.longitude || 0 });
      setPhone(player.phone || '');
      setEmail(player.email || '');
      setHandedness(player.handedness || '');
      setExperience(player.experience || '');
      setTerrainId(player.terrainId);
      setTerrainName(player.terrainName || '');
      setBoulesName(player.boules?.name || '');
      setBoulesDiameter(player.boules?.diameter?.toString() || '');
      setBoulesWeight(player.boules?.weight?.toString() || '');
    }
  }, [player]);
  ...
  const roleAnalysis = useMemo(() => {
    if (!id) return { roleCounts: {} as Record<string, number>, totalWithRoles: 0, preferredRole: '' };
    ...
    allMatches.forEach(match => {
      if (match.teamA.players.includes(id) && match.teamA.playerRoles) { const pr = match.teamA.playerRoles.find(r => r.playerId === id); if (pr && roleCounts[pr.role] !== undefined) roleCounts[pr.role]++; }
      if (match.teamB.players.includes(id) && match.teamB.playerRoles) { const pr = match.teamB.playerRoles.find(r => r.playerId === id); if (pr && roleCounts[pr.role] !== undefined) roleCounts[pr.role]++; }
    });
    const totalWithRoles = Object.values(roleCounts).reduce((a, b) => a + b, 0);
    const preferredRole = totalWithRoles > 0 ? Object.entries(roleCounts).reduce((a, b) => b[1] > a[1] ? b : a)[0] : '';
    return { roleCounts, totalWithRoles, preferredRole };
  }, [matches, id]);

  const terrainTypeIcons: Record<string, { icon: string; color: string }> = {
    'Stabilisé': { icon: 'landscape', color: '#6B7280' }, 'Graviers': { icon: 'grain', color: '#92400E' },
    'Sable': { icon: 'beach-access', color: '#D97706' }, 'Cailloux': { icon: 'terrain', color: '#6D28D9' },
    'Terre battue': { icon: 'grass', color: '#15803D' },
  };

  const terrainAnalysis = useMemo(() => {
    if (!id) return { terrainCounts: {} as Record<string, number>, totalWithTerrain: 0, preferredTerrain: '' };
    const terrainCounts: Record<string, number> = {};
    const allMatches = matches.filter(m => m.teamA.players.includes(id) || m.teamB.players.includes(id));
    allMatches.forEach(match => { if (match.terrainType) terrainCounts[match.terrainType] = (terrainCounts[match.terrainType] || 0) + 1; });
  ...
  const partnerColors = ['#3B82F6', '#8B5CF6', '#F97316', '#10B981', '#EC4899'];
  const partnerAnalysis = useMemo(() => {
    if (!id) return { partnerCounts: {} as Record<string, { count: number; name: string }>, totalWithPartners: 0, preferredPartner: { id: '', name: '', count: 0 } };
    const partnerCounts: Record<string, { count: number; name: string }> = {};
    const allMatches = matches.filter(m => m.teamA.players.includes(id) || m.teamB.players.includes(id));
    allMatches.forEach(match => {
      const inTeamA = match.teamA.players.includes(id);
      const inTeamB = match.teamB.players.includes(id);
      if (inTeamA && match.teamA.players.length > 1) { match.teamA.players.forEach((pid, idx) => { if (pid !== id) { if (!partnerCounts[pid]) partnerCounts[pid] = { count: 0, name: match.teamA.playerNames[idx] || pid }; partnerCounts[pid].count++; } }); }
      else if (inTeamB && match.teamB.players.length > 1) { match.teamB.players.forEach((pid, idx) => { if (pid !== id) { if (!partnerCounts[pid]) partnerCounts[pid] = { count: 0, name: match.teamB.playerNames[idx] || pid }; partnerCounts[pid].count++; } }); }
    });
    const totalWithPartners = Object.values(partnerCounts).reduce((a, b) => a + b.count, 0);
    let preferredPartner = { id: '', name: '', count: 0 };
    if (totalWithPartners > 0) { const best = Object.entries(partnerCounts).reduce((a, b) => b[1].count > a[1].count ? b : a); preferredPartner = { id: best[0], name: best[1].name, count: best[1].count }; }
  ...
  const pickImage = async () => {
    try {
      ...
      if (!result.canceled && result.assets[0]) { setAvatarUri(result.assets[0].uri); setAvatarChanged(true); }
  ...
  const showImageOptions = () => {
    Alert.alert(t('player', 'profilePhoto'), t('player', 'chooseOption'), [
      ...
      ...(avatarUri ? [{ text: t('player', 'removeLabel'), style: 'destructive' as const, onPress: () => { setAvatarUri(null); setAvatarChanged(true); } }] : []),
      { text: t('common', 'cancel'), style: 'cancel' as const },
    ]);
  };

  const uploadAvatar = async (): Promise<string | undefined> => {
    if (!avatarChanged) return player?.avatar;
    if (!avatarUri) return undefined;
    if (avatarUri.startsWith('http')) return avatarUri;
    try {
      setUploadingAvatar(true);
      const url = await uploadPlayerAvatar(id || 'unknown', avatarUri);
      return url || player?.avatar;
    } catch (error) { console.log('Error uploading avatar:', error); return player?.avatar; } finally { setUploadingAvatar(false); }
  };
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.header}>
          ...
          <View style={styles.headerCenter}><Text style={styles.headerTitle}>{t('player', 'editPlayer')}</Text></View>
  ...
  const handleSave = async () => {
    ...
    const st = terrains.find(t => t.id === terrainId);
    updatePlayer(id!, {
      name: name.trim(), nickname: nickname.trim() || null, avatar: avatarUrl, role,
      clubId: clubId || null, club: sc?.name || null, terrainId: terrainId || null,
      terrainName: st?.name || terrainName || null, phone: phone.trim() || null,
      email: email.trim() || null, handedness: handedness || null, experience: experience || null, boules,
  ...
  const handleDelete = () => {
    Alert.alert(t('player', 'deletePlayer'), t('player', 'deleteIrreversibleConfirm'), [
      { text: t('common', 'cancel'), style: 'cancel' },
      { text: t('common', 'delete'), style: 'destructive', onPress: () => { deletePlayer(id!); Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning); router.replace('/(tabs)/directory'); } },
    ]);
  };
    ...
    <SafeAreaView edges={['top']} style={styles.container}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <ScrollView style={styles.scrollView} contentContainerStyle={[styles.scrollContent, { paddingBottom: insets.bottom + 100 }]} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          <Animated.View entering={FadeInDown.duration(400)} style={styles.avatarSection}>
            <Pressable style={styles.avatarContainer} onPress={showImageOptions} disabled={uploadingAvatar}>
              {avatarUri ? (
                ...
                <View style={styles.avatar}>
                  <Text style={styles.avatarText}>{name.split(' ').map(n => n[0] || '').join('').toUpperCase().slice(0, 2) || '?'}</Text>
                </View>
              )}
            ...
            <Text style={styles.avatarHint}>{t('player', 'tapToEditPhoto')}</Text>
          </Animated.View>

          {/* 1. Nom */}
          <SectionCard title={t('player', 'nameLabel')} icon="person" color={theme.primary} delay={50} required>
            <TextInput style={styles.textInput} value={name} onChangeText={setName} placeholder={t('player', 'namePlaceholder')} placeholderTextColor={theme.textMuted} autoCapitalize="words" />
                ...
                try {
                  ...
                  if (status !== 'granted') {
                    ...
                    if (newStatus !== 'granted') {
                      Alert.alert(t('common', 'error'), language === 'fr' ? 'Permission de localisation refusee' : 'Location permission denied');
                      return;
                    }
                  }
                  const position = await Location.getCurrentPositionAsync({ accuracy: Location.Accuracy.Balanced });
                  const [result] = await Location.reverseGeocodeAsync({ latitude: position.coords.latitude, longitude: position.coords.longitude });
                  if (result) {
                    const newCity = result.city || result.subregion || result.region || '';
                    const newCountry = result.country || location.country || 'France';
                    setLocation({
                      address: result.street ? `${result.street}, ${newCity}` : newCity,
                      city: newCity,
                      country: newCountry,
                  ...
                  Alert.alert(t('common', 'error'), language === 'fr' ? 'Impossible de recuperer la position GPS' : 'Unable to get GPS position');
                }
              }}
            >
              <View style={[styles.autoFillIcon, { backgroundColor: '#3B82F615' }]}>
                <MaterialIcons name="my-location" size={16} color="#3B82F6" />
              </View>
              <Text style={[styles.autoFillText, { color: '#3B82F6' }]}>
                {language === 'fr' ? 'Utiliser ma position GPS actuelle' : 'Use my current GPS location'}
              </Text>
            ...
            <Pressable style={styles.pickerButton} onPress={() => { setTerrainSearch(''); setShowTerrainPicker(true); }}>
              {selectedTerrain ? (
                <View style={styles.pickerSelected}>
                  ...
                  <Pressable onPress={(e) => { e.stopPropagation(); setTerrainId(undefined); setTerrainName(''); Haptics.selectionAsync(); }} hitSlop={8}>
          ...
          {/* Preferred Role (read-only) */}
          {roleAnalysis.totalWithRoles > 0 ? (
            <Animated.View entering={FadeInDown.duration(400).delay(420)} style={styles.readOnlySection}>
              <Text style={styles.readOnlySectionTitle}>{t('player', 'preferredRole')}</Text>
              <View style={styles.readOnlyCard}>
                <View style={styles.readOnlyHeader}>
                  <View style={[styles.readOnlyIcon, { backgroundColor: roleIcons[roleAnalysis.preferredRole]?.color || theme.primary }]}>
                    <MaterialIcons name={(roleIcons[roleAnalysis.preferredRole]?.icon || 'person') as any} size={18} color="#FFF" />
                  </View>
                  <View style={styles.readOnlyInfo}>
                    <Text style={styles.readOnlyName}>{t('roles', roleAnalysis.preferredRole)}</Text>
                    <Text style={styles.readOnlyStats}>{roleAnalysis.roleCounts[roleAnalysis.preferredRole]} {t('player', 'outOf')} {roleAnalysis.totalWithRoles} {t('player', 'matchesLabel')}</Text>
                  </View>
                </View>
                <View style={styles.readOnlyBreakdown}>
                  {Object.entries(roleAnalysis.roleCounts).map(([r, count]) => {
                    const cfg = roleIcons[r];
                    const pct = roleAnalysis.totalWithRoles > 0 ? Math.round((count / roleAnalysis.totalWithRoles) * 100) : 0;
                    return (
                      <View key={r} style={styles.readOnlyBreakdownItem}>
                        <MaterialIcons name={(cfg?.icon || 'person') as any} size={12} color={cfg?.color || theme.textMuted} />
                        <Text style={styles.readOnlyBreakdownLabel}>{r.charAt(0)}</Text>
                        <View style={styles.readOnlyBarBg}><View style={[styles.readOnlyBar, { width: `${pct}%`, backgroundColor: cfg?.color || theme.primary }]} /></View>
                        <Text style={styles.readOnlyBreakdownPct}>{pct}%</Text>
                      </View>
                    );
                  })}
                </View>
                <View style={styles.readOnlyNotice}><MaterialIcons name="info-outline" size={12} color={theme.textMuted} /><Text style={styles.readOnlyNoticeText}>{t('player', 'statsAutoCalculated')}</Text></View>
              </View>
            </Animated.View>
          ) : null}

          {/* Preferred Terrain (read-only) */}
          {terrainAnalysis.totalWithTerrain > 0 ? (
            <Animated.View entering={FadeInDown.duration(400).delay(430)} style={styles.readOnlySection}>
              <Text style={styles.readOnlySectionTitle}>{t('player', 'preferredTerrain')}</Text>
              <View style={styles.readOnlyCard}>
                <View style={styles.readOnlyHeader}>
                  <View style={[styles.readOnlyIcon, { backgroundColor: terrainTypeIcons[terrainAnalysis.preferredTerrain]?.color || theme.primary }]}>
                    <MaterialIcons name={(terrainTypeIcons[terrainAnalysis.preferredTerrain]?.icon || 'sports-soccer') as any} size={18} color="#FFF" />
                  </View>
                  <View style={styles.readOnlyInfo}>
                    <Text style={styles.readOnlyName}>{t('terrainTypes', terrainAnalysis.preferredTerrain)}</Text>
                    <Text style={styles.readOnlyStats}>{terrainAnalysis.terrainCounts[terrainAnalysis.preferredTerrain]} {t('player', 'outOf')} {terrainAnalysis.totalWithTerrain} {t('player', 'matchesLabel')}</Text>
                  </View>
                </View>
                <View style={styles.readOnlyBreakdown}>
                  {Object.entries(terrainAnalysis.terrainCounts).map(([terrainType, count]) => {
                    const cfg = terrainTypeIcons[terrainType];
                    const pct = terrainAnalysis.totalWithTerrain > 0 ? Math.round((count / terrainAnalysis.totalWithTerrain) * 100) : 0;
                    return (
                      <View key={terrainType} style={styles.readOnlyBreakdownItem}>
                        <MaterialIcons name={(cfg?.icon || 'sports-soccer') as any} size={12} color={cfg?.color || theme.textMuted} />
                        <Text style={[styles.readOnlyBreakdownLabel, { width: 18 }]}>{terrainType.substring(0, 2)}</Text>
                        <View style={styles.readOnlyBarBg}><View style={[styles.readOnlyBar, { width: `${pct}%`, backgroundColor: cfg?.color || theme.primary }]} /></View>
                        <Text style={styles.readOnlyBreakdownPct}>{pct}%</Text>
                      </View>
                    );
          ...
          {/* Preferred Partner (read-only) */}
          {partnerAnalysis.totalWithPartners > 0 ? (
            <Animated.View entering={FadeInDown.duration(400).delay(440)} style={styles.readOnlySection}>
              <Text style={styles.readOnlySectionTitle}>{t('player', 'preferredPartner')}</Text>
              <View style={styles.readOnlyCard}>
                <View style={styles.readOnlyHeader}>
                  <View style={[styles.readOnlyIcon, { backgroundColor: partnerColors[0] }]}><MaterialIcons name="people" size={18} color="#FFF" /></View>
                  <View style={styles.readOnlyInfo}>
                    <Text style={styles.readOnlyName}>{partnerAnalysis.preferredPartner.name}</Text>
                    <Text style={styles.readOnlyStats}>{partnerAnalysis.preferredPartner.count} {t('player', 'outOf')} {partnerAnalysis.totalWithPartners} {t('player', 'matchesLabel')}</Text>
                  </View>
                </View>
                <View style={styles.readOnlyPartnerList}>
                  {Object.entries(partnerAnalysis.partnerCounts).sort((a, b) => b[1].count - a[1].count).slice(0, 5).map(([pid, info], idx) => {
                    const pct = partnerAnalysis.totalWithPartners > 0 ? Math.round((info.count / partnerAnalysis.totalWithPartners) * 100) : 0;
                    const color = partnerColors[idx % partnerColors.length];
                    return (
                      <View key={pid} style={styles.readOnlyPartnerItem}>
                        <View style={[styles.readOnlyPartnerAvatar, { backgroundColor: color + '20' }]}><Text style={[styles.readOnlyPartnerAvatarText, { color }]}>{info.name.charAt(0).toUpperCase()}</Text></View>
                        <View style={styles.readOnlyPartnerInfo}>
                          <Text style={styles.readOnlyPartnerName} numberOfLines={1}>{info.name}</Text>
                          <View style={styles.readOnlyPartnerBarRow}>
                            <View style={styles.readOnlyBarBg}><View style={[styles.readOnlyBar, { width: `${pct}%`, backgroundColor: color }]} /></View>
                            <Text style={styles.readOnlyPartnerPct}>{info.count} ({pct}%)</Text>
                          </View>
                        </View>
                      </View>
                    );
          ...
          {/* Stats Preview */}
          <SectionCard title={t('player', 'statistics')} icon="bar-chart" color={theme.primary} delay={450}>
            <View style={styles.statsCard}>
              <View style={styles.statRow}><Text style={styles.statLabel}>{t('player', 'matchesPlayed')}</Text><Text style={styles.statValue}>{player.stats.matchesPlayed}</Text></View>
              <View style={styles.statRow}><Text style={styles.statLabel}>{t('player', 'victories')}</Text><Text style={[styles.statValue, { color: theme.success }]}>{player.stats.wins}</Text></View>
              <View style={styles.statRow}><Text style={styles.statLabel}>{t('player', 'defeats')}</Text><Text style={[styles.statValue, { color: theme.error }]}>{player.stats.losses}</Text></View>
              <View style={[styles.statRow, { borderBottomWidth: 0 }]}><Text style={styles.statLabel}>{t('player', 'winRate')}</Text><Text style={[styles.statValue, { color: theme.primary }]}>{player.stats.winRate}%</Text></View>
            </View>
            <Text style={styles.statsNote}>{t('player', 'statsAutoCalculated')}</Text>
          </SectionCard>
        </ScrollView>

        {/* Save */}
        <View style={[styles.footer, { paddingBottom: insets.bottom + 16 }]}>
          <Pressable style={({ pressed }) => [styles.saveButton, (!name.trim() || uploadingAvatar) && styles.saveButtonDisabled, pressed && name.trim() && !uploadingAvatar && styles.saveButtonPressed]} onPress={handleSave} disabled={uploadingAvatar}>
            {uploadingAvatar ? <ActivityIndicator color="#FFF" /> : <MaterialIcons name="check" size={22} color="#FFF" />}
            <Text style={styles.saveButtonText}>{uploadingAvatar ? t('player', 'saving') : t('player', 'savePlayer')}</Text>
          </Pressable>
        </View>
      </KeyboardAvoidingView>
      ...
      <Modal visible={showTerrainPicker} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setShowTerrainPicker(false)}>
        <SafeAreaView edges={['top']} style={styles.modalContainer}>
          ...
          <Pressable style={[styles.modalPickerItem, !terrainId && styles.modalPickerItemActive]} onPress={() => { Haptics.selectionAsync(); setTerrainId(undefined); setTerrainName(''); setShowTerrainPicker(false); }}>
...
const styles = StyleSheet.create({
  ...
  // Auto-fill
  gpsUpdateBtn: { flexDirection: 'row' as const, alignItems: 'center' as const, gap: 10, backgroundColor: '#3B82F608', borderRadius: theme.borderRadius.md, padding: 12, marginTop: 10, borderWidth: 1, borderColor: '#3B82F620' },
  ...
  // Stats
  statsCard: { backgroundColor: theme.backgroundSecondary, borderRadius: theme.borderRadius.md, padding: 14 },
  statRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: theme.border },
  statLabel: { fontSize: 15, color: theme.textSecondary },
  statValue: { fontSize: 15, fontWeight: '600', color: theme.textPrimary },
  statsNote: { fontSize: 12, color: theme.textMuted, marginTop: 8, textAlign: 'center' },
  ...
  readOnlySection: { marginBottom: 14 },
  readOnlySectionTitle: { fontSize: 12, fontWeight: '600', color: theme.textSecondary, letterSpacing: 1, marginBottom: 10, paddingHorizontal: 4 },
  readOnlyCard: { backgroundColor: theme.surface, borderRadius: theme.borderRadius.lg, padding: 16, borderWidth: 1, borderColor: theme.border, ...theme.shadows.card },
  readOnlyHeader: { flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 12 },
  readOnlyIcon: { width: 40, height: 40, borderRadius: 20, alignItems: 'center', justifyContent: 'center' },
  readOnlyInfo: { flex: 1 },
  readOnlyName: { fontSize: 16, fontWeight: '700', color: theme.textPrimary },
  readOnlyStats: { fontSize: 12, color: theme.textSecondary, marginTop: 2 },
  readOnlyBreakdown: { flexDirection: 'row', gap: 8, marginBottom: 10 },
  readOnlyBreakdownItem: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: theme.backgroundSecondary, paddingVertical: 8, paddingHorizontal: 8, borderRadius: theme.borderRadius.md },
  readOnlyBreakdownLabel: { fontSize: 11, fontWeight: '600', color: theme.textSecondary, width: 12 },
  readOnlyBarBg: { flex: 1, height: 4, backgroundColor: theme.border, borderRadius: 2, overflow: 'hidden' },
  readOnlyBar: { height: '100%', borderRadius: 2 },
  readOnlyBreakdownPct: { fontSize: 11, fontWeight: '600', color: theme.textPrimary, minWidth: 28, textAlign: 'right' },
  readOnlyNotice: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingTop: 10, borderTopWidth: 1, borderTopColor: theme.border },
  readOnlyNoticeText: { fontSize: 11, color: theme.textMuted, fontStyle: 'italic' },
  readOnlyPartnerList: { gap: 8, marginBottom: 10 },
  readOnlyPartnerItem: { flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: theme.backgroundSecondary, paddingVertical: 10, paddingHorizontal: 10, borderRadius: theme.borderRadius.md },
  readOnlyPartnerAvatar: { width: 30, height: 30, borderRadius: 15, alignItems: 'center', justifyContent: 'center' },
  readOnlyPartnerAvatarText: { fontSize: 13, fontWeight: '700' },
  readOnlyPartnerInfo: { flex: 1, gap: 4 },
  readOnlyPartnerName: { fontSize: 13, fontWeight: '600', color: theme.textPrimary },
  readOnlyPartnerBarRow: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  readOnlyPartnerPct: { fontSize: 11, fontWeight: '600', color: theme.textSecondary, minWidth: 50, textAlign: 'right' },
});
